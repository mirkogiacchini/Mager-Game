...
public class Util 
{
  ...
  /**
   * restituisce il FileHandle della path specificata
   */
  public static FileHandle getHandle(String path)
  {
	FileHandle fh = null;
	try { fh = Gdx.files.internal(path); }catch(Exception e){ fh = null; }
	if(fh == null)
	 try { fh = Gdx.files.external(path); } catch(Exception e) { fh = null; }
	if(fh == null)
	 try { fh = Gdx.files.local(path); } catch(Exception e) { fh = null; };
	return fh;
  }
